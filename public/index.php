...
?>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      var wrapper = document.querySelector('.lootbox-wrapper');
      ...
      windowEl.setAttribute('data-orientation', orientation);
      ...
      function centerCard(loopIndex, itemIndex, animate, duration) {
        ...
        if (orientation === 'vertical') {
          offset = card.offsetTop + card.offsetHeight / 2 - windowEl.offsetHeight / 2;
        } else {
          offset = card.offsetLeft + card.offsetWidth / 2 - windowEl.offsetWidth / 2;
        }
        if (!animate) {
      ...
      function recenterActiveCard() {
        ...
        if (activeCard && activeCard.isConnected) {
          var rawIndex = activeCard.getAttribute('data-index');
          if (rawIndex !== null) {
            targetIndex = parseInt(rawIndex, 10) || 0;
          }
        }
        centerCard(currentLoop, targetIndex, false, 0);
      }
      ...
      if (typeof ResizeObserver !== 'undefined') {
        try {
          var orientationObserver = new ResizeObserver(function () {
            updateOrientation();
          });
          orientationObserver.observe(windowEl);
        } catch (err) {
